...
# ------------------------------------------------------------------
# Quiet mode tweaks: suppress most warnings / progress bars
# ------------------------------------------------------------------
import os
import warnings
import logging
...
# Silence Python + NeMo warnings
warnings.filterwarnings("ignore")

# Reduce root-logger verbosity
logging.getLogger().setLevel(logging.ERROR)

# Reduce NeMo logger verbosity to ERROR
nemo_logging.set_verbosity(logging.ERROR)

# Disable tqdm-based progress bars used internally by NeMo / Lhotse
os.environ["NEMO_DISABLE_BAR"] = "1"

# Optional: hide Hydra debug output
...
def main(cfg: TranscriptionConfig) -> Union[TranscriptionConfig, List[Hypothesis]]:
    ...
    # Flag to postpone model move to MPS (to avoid float64‑dtype crash)
    use_mps_later = False
    # setup GPU
    ...
    if cfg.cuda is None:
        if torch.cuda.is_available():
            ...
            logging.warning(
                ...
                "Env variable `PYTORCH_ENABLE_MPS_FALLBACK=1` should be set to avoid failures."
            )
            device = [0]
            accelerator = 'mps'
            map_location = torch.device('cpu')  # restore on CPU first
            use_mps_later = True               # move after restore
        else:
            device = 1
            accelerator = 'cpu'
    ...
    # --- move to MPS *after* restoration to avoid float64 tensors ---
    if use_mps_later:
        asr_model = asr_model.to(device='mps', dtype=torch.float32)
